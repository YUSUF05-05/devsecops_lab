...
# Fixed: use raw strings (r"...") to avoid backslash escape issues on Windows
# Fixed: updated folder name from session1 -> part1
TARGET_FILE = r"..\part1\app_vulnerable.py"
SECURE_FILE = r"..\part1\app_secure.py"
...
def run_safety_check():
    ...
    # Fixed: updated from deprecated 'safety check' to 'safety scan'
    result = subprocess.run(
        ["safety", "scan", "-r", REQUIREMENTS_FILE, "--output", "json"],
        capture_output=True, text=True
    )
    try:
        output = result.stdout.strip()
        if not output:
            print("Safety output was empty. Check that safety is installed:")
        ...
        if vulnerabilities:
            ...
            for v in vulnerabilities:
                print(f"  - {v.get('package_name')} {v.get('analyzed_version')}: {str(v.get('advisory', ''))[:80]}")
        else:
...
def generate_report(vulnerable_data: dict, secure_data: dict):
    ...
    print("  - Pickle deserialize   --> JSON with validation")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(TARGET_FILE):
        ...
        print("Make sure you are running this from the part2\\ directory.")
        sys.exit(1)
